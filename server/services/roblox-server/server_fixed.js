...
gameServer.on('message', (msg, rinfo) => {
  logger.debug(`Game server received ${msg.length} bytes from ${rinfo.address}:${rinfo.port}`);
  
  // Parse the message and handle game-specific logic
  try {
    ...
    switch (message.type) {
      case 'heartbeat':
        ...
        gameServer.send(response, 0, response.length, rinfo.port, rinfo.address);
        break;
        
      case 'player_connect':
        // Handle player connection request
        const connectResponse = JSON.stringify({ 
          type: 'connect_response',
          success: !gameState.isFull(),
          maxPlayers: config.maxPlayers,
          currentPlayers: gameState.currentPlayers
        });
        gameServer.send(connectResponse, 0, connectResponse.length, rinfo.port, rinfo.address);
        break;
...
gameServer.on('listening', () => {
  ...
  logger.info(`Game server listening on ${address.address}:${address.port}`);
});

gameServer.bind(config.gamePort);
...
process.on('SIGTERM', () => {
  ...
  gameServer.close(() => {
    logger.info('Game server heruntergefahren');
  });
  server.close(() => {
    logger.info('HTTP server heruntergefahren');
    process.exit(0);
  });
});
